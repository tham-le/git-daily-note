...
def generate_config(config_path=None):
    ...
    example["_comment"] = {
        ...
        "gitlab_group": "GitLab group path for --team mode (e.g. 'kyber-labs/kyber.stream-private')",
...
class GitLabSync:
    ...
    def fetch_mr_discussions(self):
        ...
        discussed = sum(1 for v in self.mr_discussions.values() if v["pending"] > 0 or v["answered"] > 0)
    ...
    def _get_repo_short_name(item):
        """Extract short repo name from references.full, e.g. 'kyber-desktop' from full path"""
        ref = item.get("references", {}).get("full", "")
        # e.g. kyber-labs/kyber.stream-private/apps/kyber-desktop!12 -> kyber-desktop
        parts = ref.split("/")
        if len(parts) >= 2:
            last = parts[-1]
    ...
    def format_markdown(self, date, todos, diff=None):
        ...
        lines.append(f"- {pending_discussion_count} pending threads | {len(todos['build_failed'])} build failures | {len(conflict_mrs)} conflicts")
